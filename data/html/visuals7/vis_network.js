...
function buildNetworkData(data, filters={}) {
  ...
  for(const [sn, ids] of Object.entries(subnetMap)){
    const sid = 'subnet-'+sn, color = subnetColor(sn);
    allNodes.push({ id:sid, label:sn, group:'subnet', shape:'ellipse', color, font:{color:"#fff"}, physics:false });
    ids.forEach(i=> allEdges.push({ from:sid, to:i, color:{color,opacity:0.35}, dashes:true, width:1 }));
  }
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const hosts = nodes.filter(n=>['normal','docker'].includes(n.group));
